...
if (isset($_POST['edit_room'])) {
    ...
    if ($roomId > 0) {
        $beddingValue = (string)max(1, $bedding);
        $stmt = $conn->prepare('UPDATE room SET type=?, bedding=? WHERE id=?');
        if ($stmt) {
            $stmt->bind_param('ssi', $type, $beddingValue, $roomId);
            $stmt->execute();
            $stmt->close();
        }
    }
...
$roomTypeChoices = ['Habitación Sencilla', 'Habitación Doble', 'Habitación Múltiple'];
$capacityChoices = [1, 2, 3, 4, 5, 6];
?>
<!DOCTYPE html>
<html lang="es">
<head>
              ...
              <div class="card-body" id="room-<?php echo $row['id']; ?>">
                <div class="d-flex justify-content-between align-items-center mb-3">
                  ...
                  <div class="text-end">
                    ...
                    <p class="mb-0 text-muted"><?php echo htmlspecialchars($row['type']); ?> · <?php echo htmlspecialchars(formatGuestCapacity($row['bedding'])); ?></p>
                  </div>
                </div>
                  ...
                  <div class="row g-2">
                    <div class="col-6">
                      <select name="type" class="form-select form-select-sm">
                        <?php foreach ($roomTypeChoices as $typeOption): ?>
                          <option value="<?php echo htmlspecialchars($typeOption); ?>" <?php echo $row['type'] === $typeOption ? 'selected' : ''; ?>><?php echo htmlspecialchars($typeOption); ?></option>
                        <?php endforeach; ?>
                        <?php if ($row['type'] !== '' && !in_array($row['type'], $roomTypeChoices, true)): ?>
                          <option value="<?php echo htmlspecialchars($row['type']); ?>" selected><?php echo htmlspecialchars($row['type']); ?></option>
                        <?php endif; ?>
                      </select>
                    </div>
                    <div class="col-6">
                      <select name="bedding" class="form-select form-select-sm">
                        <?php foreach ($capacityChoices as $capacity): ?>
                          <option value="<?php echo $capacity; ?>" <?php echo (int)$row['bedding'] === $capacity ? 'selected' : ''; ?>><?php echo formatGuestCapacity($capacity); ?></option>
                        <?php endforeach; ?>
                        <?php $currentCapacity = (int)$row['bedding']; ?>
                        <?php if ($currentCapacity > 0 && !in_array($currentCapacity, $capacityChoices, true)): ?>
                          <option value="<?php echo $currentCapacity; ?>" selected><?php echo formatGuestCapacity($currentCapacity); ?></option>
                        <?php endif; ?>
                      </select>
                    </div>
                  </div>
